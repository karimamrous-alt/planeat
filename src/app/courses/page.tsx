...
import { FAMILLE_ID, getMondayOfWeek, formatSemaine, JOURS, CUISINE_CONFIG } from '@/lib/utils'
import type { Recette, DayMenu } from '@/lib/types'

// ── Modal ingrédients ─────────────────────────────────────────────────────────

function Modal({ recette: r, onClose }: { recette: Recette; onClose: () => void }) {
  const cfg = CUISINE_CONFIG[r.cuisine] ?? { emoji: '🍴', colorClass: 'text-gray-600', bgClass: 'bg-gray-50 border-gray-200', ph: 'ph-default' }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 px-0 sm:px-4" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md rounded-t-4xl sm:rounded-3xl shadow-card-lg flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
        {/* Photo placeholder */}
        ...
        <div className="p-5 border-b" style={{ borderColor: '#F0E6DC' }}>
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <h2 className="font-display font-bold text-lg leading-tight" style={{ color: '#2C1810' }}>{r.nom}</h2>
              <p className="text-xs mt-1" style={{ color: '#8B5E3C' }}>{r.personnes} pers.{r.calories > 0 ? ` · ${r.calories} kcal` : ''}</p>
            </div>
        ...
        <div className="p-4 border-t grid grid-cols-3 gap-2" style={{ borderColor: '#F0E6DC' }}>
          <a href={`https://wa.me/?text=${encoded}`} target="_blank" rel="noopener noreferrer"
            className="flex flex-col items-center gap-1 bg-green-50 hover:bg-green-100 border border-green-200 rounded-2xl py-3 px-2 transition-colors">
            <span className="text-xl">📱</span>
            <span className="text-xs font-semibold text-green-700">WhatsApp</span>
          </a>
          <a href={`sms:?body=${encoded}`}
            className="flex flex-col items-center gap-1 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-2xl py-3 px-2 transition-colors">
            <span className="text-xl">💬</span>
            <span className="text-xs font-semibold text-blue-700">SMS</span>
          </a>
          <button onClick={imprimer}
            className="flex flex-col items-center gap-1 bg-orange-50 hover:bg-orange-100 border border-orange-200 rounded-2xl py-3 px-2 transition-colors">
            <span className="text-xl">🖨️</span>
            <span className="text-xs font-semibold text-orange-700">Imprimer</span>
          </button>
        </div>
      </div>
    </div>
...
export default function Courses() {
  ...
  if (!hasMenu) return (
    <div className="text-center py-16 fade-in">
      ...
      <h2 className="font-display text-xl font-bold mb-2" style={{ color: '#2C1810' }}>Aucun menu cette semaine</h2>
      <p className="text-sm mb-6" style={{ color: '#8B5E3C' }}>Générez et validez d&apos;abord votre menu.</p>
      <Link href="/menus"
  ...
  return (
    <div className="fade-in space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold" style={{ color: '#2C1810' }}>🛒 Liste de courses</h1>
        <p className="text-sm mt-0.5" style={{ color: '#8B5E3C' }}>
          Semaine du {formatSemaine(semaine)} · {recettes.length} recette{recettes.length > 1 ? 's' : ''}
        </p>
      </div>

      <p className="text-sm" style={{ color: '#8B5E3C' }}>
        Appuyez sur une recette pour voir ses ingrédients et les partager.
      </p>
      ...
      {recettes.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {recettes.map(rec => {
            ...
            return (
              ...
              >
                {/* Photo placeholder */}
                <div className={`h-20 flex items-center justify-center text-3xl ${cfg.ph}`}>
                  {cfg.emoji}
                </div>
                <div className="p-4">
                  <p className={`text-xs font-bold mb-1 ${cfg.colorClass}`}>{cfg.emoji} {rec.cuisine}</p>
                  <p className="font-bold text-sm leading-snug truncate" style={{ color: '#2C1810' }}>{rec.nom}</p>
                  ...
                  <p className="text-xs mt-2" style={{ color: '#8B5E3C' }}>
        ...
        <div className="text-center py-10 bg-white rounded-3xl" style={{ boxShadow: '0 2px 16px rgba(44,24,16,0.08)' }}>
          <p className="text-sm" style={{ color: '#8B5E3C' }}>Aucune recette dans le menu.</p>
        </div>
      )}
