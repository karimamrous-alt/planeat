    ...
    <div className="fade-in space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{membres.length} membres · Régime {famille?.preferences?.regime ?? '—'}</p>
        </div>
          ...
          {!editFam ? (
            ...
            <div className="flex gap-2">
              ...
              <button onClick={sauvegarderFamille} disabled={saving}
                className="text-sm bg-green-700 text-white px-3 py-1 rounded-lg hover:bg-green-800 disabled:opacity-50">
        ...
        {editFam ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Régime', value: famille?.preferences?.regime ?? '—' },
      ...
      {/* Cuisines */}
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <h2 className="font-bold text-gray-700 mb-4">Cuisines disponibles</h2>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={ajouterMembre}
            className="text-sm bg-green-700 text-white px-4 py-2 rounded-xl hover:bg-green-800 transition-colors font-medium">
            + Ajouter
          </button>
        </div>
      ...
      <div className="flex items-start justify-between mb-3">
        {editing ? (
          <input value={prenom} onChange={e => setPrenom(e.target.value)}
            className="font-bold text-gray-800 border-b border-green-400 focus:outline-none text-base flex-1 mr-2" autoFocus />
        ) : (
